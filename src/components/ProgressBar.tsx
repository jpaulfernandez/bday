...
export default function ProgressBar({
  ...
  return (
    <div
      ...
      className={`fixed top-3 left-3 right-28 sm:top-6 sm:left-6 sm:right-auto z-40 w-auto sm:w-full max-w-none sm:max-w-[380px] bg-pearl/95 backdrop-blur-md border-2 border-white/80 rounded-3xl ${
        isCollapsed ? "py-2 px-3.5 cursor-pointer hover:bg-white/95 active:scale-95" : "p-3 sm:p-4"
      } shadow-xl text-reef transition-all duration-300 pointer-events-auto select-none`}
    >
      {/* Header Bar */}
      <div className={`flex items-center justify-between gap-2 text-xs sm:text-sm font-heading font-bold ${!isCollapsed ? "mb-2" : ""}`}>
        <span className="flex min-w-0 items-center gap-1.5 text-reef">
          <span className="text-lg sm:text-xl animate-bounce-gentle inline-block filter drop-shadow-sm">🍜</span>
          <span className="truncate">
            <span className="font-extrabold text-reef">Birthday Ramen</span>: {collectedCount}/{totalCount}
            {!isCollapsed && <span className="hidden sm:inline"> Ingredients</span>}
          </span>
        </span>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className="text-coral font-bold flex items-center gap-1 bg-coral/10 px-2 py-0.5 rounded-full text-[11px] sm:text-xs">
          ...
          <button
            ...
            aria-label={isCollapsed ? "Expand mission board" : "Collapse mission board"}
            className="p-1 rounded-full bg-seafoam/30 hover:bg-seafoam/50 text-reef flex items-center justify-center transition-all cursor-pointer"
          >
            {isCollapsed ? <ChevronDown className="w-4 h-4 text-coral font-bold" /> : <ChevronUp className="w-4 h-4 text-coral font-bold" />}
          </button>
        </div>
      </div>
